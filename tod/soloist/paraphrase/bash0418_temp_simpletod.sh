...
## warning: you should run this file after running `bash0418_pollution_simpletod.sh` in simpletod directory.
...
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> tempering + exp+bp+multi-target 5"
...
do
    export save_model_path="${root_dir}/data/mt5-tempering-exp-bp_fraction1.0_prate${prate}/"
    ...
    export root_dir=/home/liangzi/TEMP/soloist/paraphrase
    for num_id in ${id_list[*]};
    do
	export dataset_name="multiwoz-2.1-"

	export inference_model_name="/data/mt5-tempering-exp-bp_fraction1.0_prate${prate}/"
	export save_file_path="simpletod-mt5-tempering-exp-bp_fraction1.0_prate${prate}_numid${num_id}.txt"
	export load_file_path=${root_dir}/../simpletod/${rate}_simpletod_predict_files_${num_id}.txt
...
for prate in ${prate_list[*]};
do
    export save_model_path="${root_dir}/data/mt5-tempering-wta-bp_fraction1.0_prate${prate}/"
	...
	export inference_model_name="/data/mt5-tempering-wta-bp_fraction1.0_prate${prate}/"
	export save_file_path="simpletod-mt5-tempering-wta-bp_fraction1.0_prate${prate}_numid${num_id}.txt"
	export load_file_path=${root_dir}/../simpletod/${rate}_simpletod_predict_files_${num_id}.txt
