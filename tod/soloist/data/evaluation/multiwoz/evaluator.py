...
class MultiWozEvaluator(BaseEvaluator):
    ...
    def _get_requestables_and_venues(self, beliefs, responses, dialog_booked_domains):
        ...
        requestables = {'phone', 'address', 'postcode', 'reference', 'id'} # request this slots..
        provided_requestables = defaultdict(lambda: set()) # if not such key, return a empty set.
        venue_offered = defaultdict(lambda: []) # if no such key, return a empty list.
        for i, (belief, response, booked_domains) in enumerate(zip(beliefs, responses, dialog_booked_domains)):
            database_results = self.db(belief, return_results=True) # qeury database
            current_requestables = set(self.label_regex.findall(response))# slots in current response
            ...
            # delete legacy in current requestables.
            ...
            for requestable_candidate in legacy_requestables:
                ...
                if slot == 'reference' and domain in {'restaurant', 'hotel', 'train'}:
                    if domain not in booked_domains:
                        # we just focus on any elements in dialgoue_booked_domains
                        continue
    ...
    def _evaluate_generated_dialogue(self, real_requestables, provided_requestables,
        ...
        if match == 1.0:
            ...
            ## provided_requestables is the same as real_requestables
