...
class Trainer:
    ...
    def train(self):
        ...
        if self.args.use_rettig==1:
            ...
            metrics = dict(loss=Mean(),bp_state_loss=Mean(),bp_tem_loss=Mean(),
                              lm_loss=Mean(), c_loss=Mean(),
                       bs_loss=Mean(), res_loss=Mean(), bs_acc=LanguageAccuracy(),
                       res_acc=LanguageAccuracy(), c_acc=BinaryAccuracy())
            ...
            for i, batch in enumerate(tqdm(self.train_dataloader,
                ...
                if self.args.use_rettig==1:
                    def _train_step(batch):
                        ...
                        def forward(batch):
                            output = self.model(**batch)
                            sbploss,tbploss,beliefloss,
                            responseloss,responsece,consistency_loss=output[:6]
                            # loss = belief_loss + response_loss + consistency_loss
                            loss = sbploss+tbploss+beliefloss+responseloss+consistency_loss
                            return loss, output[:8]
                        ...
                        metrics['loss'](loss)
                        metrics['bp_state_loss'](sbploss)
                        metrics['bp_tem_loss'](tbploss)
                        metrics['lm_loss'](belief_loss + response_ce)
                        metrics['bs_loss'](belief_loss)
                        metrics['res_loss'](response_ce)
                        metrics['c_loss'](consistency_loss)
                        metrics['bs_acc'](output[4+2], batch['belief_labels'])
                        metrics['res_acc'](output[4+2], batch['response_labels'])
                        metrics['c_acc'](output[5+2], batch['consistency_labels'])
