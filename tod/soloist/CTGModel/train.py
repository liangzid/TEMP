...
class Trainer:
    ...
    def _initialize_dataloaders(self) -> torch.utils.data.Dataset:
        ...
        elif self.args.backinference == 4:
            self.logger.info('mode: CTG SCGPT')
            self.logger.info('YOU CAN ONLY USE CTG-SCGPT IN THIS DIRECTORY.')
            backinference_transformation = data.CTGScgptTransformation(self.tokenizer,make_mask=self.args.vision_mask)
            transform=[data.InsertLabelsTransformation(),backinference_transformation]
            train_transform = list(transform)
