...
if True:
    class Conversation:
        ...
        def __init__(self, text: str = None,dialogue_actions=None, conversation_id: UUID = None):
...
def get_actss(conversations):
    resutls=[]
    for c in conversations:
        resutls.append(c.dialogue_actions)
        return resutls
...
class SoloistConversationalPipeline(transformers.Pipeline):
    ...
    def __call__(self, conversations: Union[SoloistConversation, Conversation, List[Union[SoloistConversation, Conversation]]]):
        ...
        with self.device_placement():
            ...
            dialogue_actions=get_actss(conversations)
            ...
            delex_responses = self.predictor.predict_response(dialogue_actions)
            responses = delex_responses

            output = []
            for conversation_index, (conversation, delex) \
                    in enumerate(zip(conversations, delex_responses)):
                conversation.mark_processed()
                conversation.append_response(delex)
    ...
    @staticmethod
    def _get_actss_from_conversations(conversations):
        def _get_acts_from_conversation(conversation):
            user = list(conversation.past_user_inputs)
            if conversation.new_user_input is not None:
                user.append(conversation.new_user_input)
            system = list(conversation.generated_responses)
