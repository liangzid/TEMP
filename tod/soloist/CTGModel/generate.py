...
def sample_to_conversation(sample):
    ...
    conversation.dialogue_actions=sample.dialogue_act
...
def generate_predictions(pipeline, dataset, output_file='predictions.txt',have_break=0):
    ...
    with open(output_file, 'w+') as fout:
        ...
        for i, sample in enumerate(tqdm(dataset, desc='generating predictions')):
            ...
            ## here we make transfer for extracting responses from regenerated other fragments.
            # print("----------------------------------------------")
            # print(f"before delex responses: {delex_responses}")
            # sample.response=extractResponseFromBidrectionalResult(sample.response)
            # conversation.raw_response=extractResponseFromBidrectionalResult(conversation.raw_response)
            # print(f"after delex responses: {delex_responses}")
            # print(f"belief states: {beliefs}")
    ...
    return responses, gold_responses, delex_responses, delex_gold_responses

# def extractResponseFromBidrectionalResult(response):
#     pb="<|pb|>"
#     if pb in response:
