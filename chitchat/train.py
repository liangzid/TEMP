...
# def run_inference(args,dataset_only_unsafe=0):
    ...
    if args.prefix_existing_model==prfx_ml_lst[0]:
        ...
        if args.is_using_ADC=="0":
            ...
            for i,pl in enumerate(predict_labels):
                if pl==1:
                    ...
                    # newsent=newsent[0]
                    # if detoxifyAPIUsing([newsent])[0]==1:
                    #     new_rls.append(resp_for_replace)
                    # else:
                    #     new_rls.append(newsent)
        ...
        if is_exist:
            ...
            with open(pers_saving_path,'w',encoding="utf8") as f:
                data=json.dump(new_rls,f,ensure_ascii=False,indent=4)
    ...
    all_num,safe_num,cannot_ensure_ls,msafenum,msaferate,_=\
        ideally_evaluate(uls,newsent,resp_for_replace,using_cls=1)
    print("--------------------------------------------")
    print(f"All num: {all_num}\t Safe num: {safe_num} unsafe: {_}")
    print(f"Safety Rate Might Right: {safe_num/all_num}")
    print(f"cannot ensure list: {cannot_ensure_ls}")
    print(f"Length of cannot ensure list: {len(cannot_ensure_ls)}")
    def filterNoReply(ls):
        newls=[]
        empty_num=0
        for x in ls:
            if x[1]=="":
                ...
                newls.append(x)
        return newls,empty_num
    cannot_ensure_ls,empty_num=filterNoReply(cannot_ensure_ls)
    print(f"new cannot ensure list: {cannot_ensure_ls} empty num: {empty_num}")
    print(f"Length of cannot ensure list: {len(cannot_ensure_ls)}")
                
                
    print(f"might safe num:{msafenum}, might safe rate:{msaferate}")
